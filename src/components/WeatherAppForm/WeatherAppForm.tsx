...
const WeatherForm = () => {
  ...
  const handleCheckboxChange = () => {
    ...
    // If useLocation is checked, retrieve location based on IP info and populate fields
    if (!formData.useLocation) {
  ...
  const handleResultsClick = () => {
    setActiveButton("results");
    setCarouselIndex(0); // Reset to DayView tab
  };
...
// Update fetchFavorites to also fetch and set filled star states correctly
useEffect(() => {
  fetchFavorites();
}, []); 
  ...
  const fetchAutocompleteSuggestions = async (input: string) => {
    try {
      ...
      // Process and transform the data directly in this function
